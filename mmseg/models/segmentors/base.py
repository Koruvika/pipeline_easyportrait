...
class BaseSegmentor(BaseModule, metaclass=ABCMeta):
    ...
    def forward(self, img, img_metas, return_loss=True, **kwargs):
        ...
        if return_loss:
            ...
            test_results = self.forward_test(img, img_metas, **kwargs)
            if isinstance(test_results, list):
                new_test_results = []
                for result in test_results:
                    new_test_results.append(result.astype(np.uint8))
                return new_test_results
            elif isinstance(test_results, torch.Tensor):
                return test_results.to(torch.uint8)
            elif isinstance(test_results, np.ndarray):
                return test_results.astype(np.uint8)
            else:
                raise TypeError('List or Array or Tensor??')
