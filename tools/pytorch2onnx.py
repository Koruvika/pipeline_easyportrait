...
def _demo_mm_inputs(input_shape, num_classes):
    ...
    mm_inputs = {
        'imgs': torch.FloatTensor(imgs).cuda().requires_grad_(True),
        'img_metas': img_metas,
        'gt_semantic_seg': torch.LongTensor(segs).cuda()
    }
    return mm_inputs
...
def pytorch2onnx(model,
    ...
    if verify:
        ...
        for rep in load_bar:
            ...
            load_bar.set_description(f"Time: {curr_time}")
            timings[rep] = curr_time

        mean_syn = np.sum(timings) / repetitions
        print(f"Pytorch: {mean_syn}")
        ...
        for _ in range(10):
            onnx_result = sess.run(
                None, {net_feed_input[0]: inputs})[0][0]
...
def parse_args():
    ...
    parser.add_argument(
        '--verify', action='store_false', help='verify the onnx model')
    parser.add_argument(
        '--output-file', type=str,
        default='/home/duongdhk/backup/duongdhk/checkpoints/fpn/fpn.resnet50.512x512.onnx',
